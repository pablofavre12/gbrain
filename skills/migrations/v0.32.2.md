...
3. Refuses to run when any source's local_path has uncommitted git
   ...
   A configured `local_path` must also already exist as a directory. Do not
   point it at an ephemeral release filesystem: use `NULL` for a thin client,
   or run the migration from a durable canonical checkout.
...
- **Don't configure `sources.local_path` with a missing or ephemeral path.**
  The migration fails closed when the directory is absent. Thin clients use
  `NULL`; writable migrations run from the durable canonical checkout.
